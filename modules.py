...
class MemoryModule(nn.Module):
    ...
    def set_memory(self, node_idxs, values):
        self.memory[node_idxs, :] = values.float()
...
class MemoryOperation(nn.Module):
    ...
    def __init__(self, updater_type, memory, e_feat_dim, temporal_encoder):
        ...
        self.memory = memory
        self.memory_dim = self.memory.hidden_dim
        self.temporal_encoder = temporal_encoder
        self.message_dim = self.memory_dim+self.memory_dim + \
            e_feat_dim+self.temporal_encoder.dimension
        self.fc = nn.Linear(self.message_dim, self.memory_dim)
        self.batchnorm = nn.BatchNorm1d(num_features=self.memory_dim)
        
        if updater_type == 'ssm':
          self.updater = Mamba(d_model = self.memory_dim, # Model dimension d_model
    ...
    def msg_fn_cat(self, edges):
        # print("edges shape: ",edges.data['timestamp'].shape)
        ...
        # print("Total Messages: ", ret.shape)
    ...
    def update_memory(self, nodes):
        ...
        # print("Before update: ", nodes.data['message_bar'].shape)


        # seq = torch.cat((nodes.data['message_bar'].float(),nodes.data['memory'].float()), dim = 0)
        new_tensor = torch.stack((nodes.data['memory'].float(),torch.relu(self.batchnorm(self.fc(nodes.data['message_bar'].float())))), dim=1)
        ret = self.updater(new_tensor)
        # ret = new_tensor
        # print("After update: ", ret.shape)
        second_tensor = ret[:, 1, :]
        # print("After extraction: ", second_tensor.shape)
        # ret = self.updater(
        #     nodes.data['message_bar'].float(), nodes.data['memory'].float())
        return {'memory': second_tensor}
